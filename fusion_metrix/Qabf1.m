...
% 
% % strA=imread('7left.jpg');
% % strB=imread('7right.jpg');
% % f=imread('C:\999.jpg');
% % f=f(:,:,1);
% % strF=f;
%strF=strF(:,:,1);
...
for i=1:M
    for j=1:N
        if ( SAx(i,j) == 0 ) aA(i,j) = pi/2;
            ...
            aA(i,j) = atan(SAy(i,j)/(SAx(i,j)+eps));
        end
    end
end
...
for i=1:M
    for j=1:N
        if ( SBx(i,j) == 0 ) aB(i,j) = pi/2;
            ...
            aB(i,j) = atan(SBy(i,j)/(SBx(i,j)+eps));
        end
    end
end
...
for i=1:M
    for j=1:N
        if ( SFx(i,j) == 0 ) aF(i,j) = pi/2;
            ...
            aF(i,j) = atan(SFy(i,j)/(SFx(i,j)+eps));
        end
    end
end
...
for i=1:M
    for j=1:N
        ...
        if ( gA(i,j) > gF(i,j))  GAF(i,j) = gF(i,j)/(gA(i,j)+eps);
        else
            if ( gA(i,j) == gF(i,j) )  GAF(i,j) = gF(i,j);
            else
                GAF(i,j) = gA(i,j) / (gF(i,j)+eps);
            end
        end 
...
for i=1:M
    for j=1:N
        ...
        if ( gB(i,j) > gF(i,j))  GBF(i,j) = gF(i,j)/(gB(i,j)+eps);
        else
            if ( gB(i,j) == gF(i,j) )  GBF(i,j) = gF(i,j);
            else
                GBF(i,j) = gB(i,j) / (gF(i,j)+eps);
            end
        end 
